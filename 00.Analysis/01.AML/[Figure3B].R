...
TF_CR <- subset(TF, TF$m.value <= -.5 & TF$meth.diff >= 5)
TF_CR <- rbind(TF_CR, TF_NR)
Odd_CR <- TF_CR$OR
...
#------------------------------------------------------------------------------#
RNA_colors <- ifelse(TF_CR$m.value > 0, "forestgreen", "firebrick1")
Meth_colors <- ifelse(TF_CR$meth.diff > 0, "forestgreen", "firebrick1")
#------------------------------------------------------------------------------#
row_right = rowAnnotation('Gene Methylation\n(beta value)' = anno_barplot(as.vector(TF_CR$meth.diff),
...
row_left = rowAnnotation('Gene Expression\n(log2fc)' = anno_barplot(as.vector(TF_CR$m.value),
                                                                    gp = gpar(fill=Meth_colors),
                                                    ylim=c(-2.5,2.5),
                                                    border=F,
                                                    bar_width = 0.8,
...
#------------------------------------------------------------------------------#
pdf('Figure3B.pdf', height=8, width=8)
draw(Plot, 
     heatmap_legend_side='bottom',
     row_split = rep(c(seq(1, nrow(Odd_CR))), 1))
draw(lgd, x = unit(6, "cm"), y = unit(3, "cm"))
dev.off()
#------------------------------------------------------------------------------#
